#!/usr/bin/env python3
"""propertyclaw tenants domain module.
...
REQUIRED_TABLES = ["company", "customer", "propertyclaw_property", "propertyclaw_application"]
SKILL = "prop-propertyclaw-tenants"
...
def add_application(conn, args):
    ...
    audit(conn, SKILL, "prop-add-application", "propertyclaw_application", app_id,
...
def get_application(conn, args):
    ...
    row = conn.execute(
        """SELECT a.*, p.name as property_name, u.unit_number
           FROM propertyclaw_application a
           JOIN propertyclaw_property p ON a.property_id = p.id
           LEFT JOIN propertyclaw_unit u ON a.unit_id = u.id
           WHERE a.id = ?""",
        (args.application_id,)).fetchone()
    if not row:
        err(f"Application {args.application_id} not found")
...
def approve_application(conn, args):
    ...
    # Update application
    conn.execute(
        "UPDATE propertyclaw_application SET status = 'approved', customer_id = ?, updated_at = datetime('now') WHERE id = ?",
        (customer_id, args.application_id))

    audit(conn, SKILL, "prop-approve-application", "propertyclaw_application", args.application_id,
...
def deny_application(conn, args):
    ...
    conn.execute(
        "UPDATE propertyclaw_application SET status = 'denied', denial_reason = ?, updated_at = datetime('now') WHERE id = ?",
    ...
    conn.execute(
        """INSERT INTO propertyclaw_adverse_action
    ...
    audit(conn, SKILL, "prop-deny-application", "propertyclaw_application", args.application_id,
...
def add_screening(conn, args):
    ...
    conn.execute(
        """INSERT INTO propertyclaw_screening_request
    ...
    if app["status"] == "received":
        conn.execute("UPDATE propertyclaw_application SET status = 'screening', updated_at = datetime('now') WHERE id = ?",
                     (args.application_id,))
...
def list_screenings(conn, args):
    if args.application_id:
        rows = conn.execute(
            "SELECT * FROM propertyclaw_screening_request WHERE application_id = ? ORDER BY screening_type",
            (args.application_id,)).fetchall()
    else:
        rows = conn.execute(
            "SELECT * FROM propertyclaw_screening_request ORDER BY created_at DESC").fetchall()
...
def add_document(conn, args):
    ...
    conn.execute(
        """INSERT INTO propertyclaw_tenant_document
...
def list_documents(conn, args):
    ...
    wc = " AND ".join(where)
    limit = int(args.limit); offset = int(args.offset)
    total = conn.execute(f"SELECT COUNT(*) FROM propertyclaw_tenant_document WHERE {wc}", params).fetchone()[0]
    rows = conn.execute(
        f"SELECT * FROM propertyclaw_tenant_document WHERE {wc} ORDER BY uploaded_at DESC LIMIT ? OFFSET ?",
        params + [limit, offset]).fetchall()
...
# ---------------------------------------------------------------------------
ACTIONS = {
    "prop-add-application": add_application,
    "prop-update-application": update_application,
    "prop-get-application": get_application,
    "prop-list-applications": list_applications,
    "prop-approve-application": approve_application,
    "prop-deny-application": deny_application,
    "prop-add-screening": add_screening,
    "prop-get-screening": get_screening,
    "prop-list-screenings": list_screenings,
    "prop-add-document": add_document,
    "prop-list-documents": list_documents,
    "prop-delete-document": delete_document,
}
