#!/usr/bin/env python3
"""propertyclaw properties domain module.

Property and unit management for PropertyClaw. Manages properties, units,
amenities, and photos. Imported by the unified propertyclaw db_query.py router.
"""
import json
import os
import sqlite3
...
# ---------------------------------------------------------------------------
REQUIRED_TABLES = ["company", "propertyclaw_property"]
SKILL = "prop-propertyclaw-properties"

_t_company = Table("company")
_t_property = Table("propertyclaw_property")
_t_unit = Table("propertyclaw_unit")
_t_amenity = Table("propertyclaw_amenity")
_t_photo = Table("propertyclaw_property_photo")
...
def add_property(conn, args):
    ...
    prop_name = get_next_name(conn, "propertyclaw_property")

    try:
        conn.execute(
            """INSERT INTO propertyclaw_property
    ...
    audit(conn, SKILL, "prop-add-property", "propertyclaw_property", prop_id,
...
def list_properties(conn, args):
    ...
    total = conn.execute(
        f"SELECT COUNT(*) FROM propertyclaw_property p WHERE {where_clause}",
        params).fetchone()[0]

    limit = int(args.limit)
    offset = int(args.offset)
    rows = conn.execute(
        f"""SELECT p.*, (SELECT COUNT(*) FROM propertyclaw_unit u WHERE u.property_id = p.id) as unit_count
            FROM propertyclaw_property p WHERE {where_clause}
...
def add_unit(conn, args):
    ...
    unit_name = get_next_name(conn, "propertyclaw_unit")

    try:
        conn.execute(
            """INSERT INTO propertyclaw_unit
    ...
    audit(conn, SKILL, "prop-add-unit", "propertyclaw_unit", unit_id,
...
def list_units(conn, args):
    ...
    total = conn.execute(
        f"SELECT COUNT(*) FROM propertyclaw_unit u WHERE {where_clause}",
        params).fetchone()[0]

    limit = int(args.limit)
    offset = int(args.offset)
    rows = conn.execute(
        f"""SELECT u.* FROM propertyclaw_unit u WHERE {where_clause}
...
# ---------------------------------------------------------------------------
ACTIONS = {
    "prop-add-property": add_property,
    "prop-update-property": update_property,
    "prop-get-property": get_property,
    "prop-list-properties": list_properties,
    "prop-add-unit": add_unit,
    "prop-update-unit": update_unit,
    "prop-get-unit": get_unit,
    "prop-list-units": list_units,
    "prop-add-amenity": add_amenity,
    "prop-list-amenities": list_amenities,
    "prop-delete-amenity": delete_amenity,
    "prop-add-photo": add_photo,
    "prop-list-photos": list_photos,
    "prop-delete-photo": delete_photo,
}
