#!/usr/bin/env python3
"""propertyclaw leases domain module.

Lease lifecycle, rent schedules, late fees, and renewals for PropertyClaw.
...
REQUIRED_TABLES = ["company", "customer", "propertyclaw_property", "propertyclaw_unit", "propertyclaw_lease"]
SKILL = "prop-propertyclaw-leases"
...
def add_lease(conn, args):
    ...
    lease_name = get_next_name(conn, "propertyclaw_lease")

    try:
        conn.execute(
            """INSERT INTO propertyclaw_lease
    ...
    audit(conn, SKILL, "prop-add-lease", "propertyclaw_lease", lease_id,
...
def list_leases(conn, args):
    ...
    wc = " AND ".join(where)
    total = conn.execute(f"SELECT COUNT(*) FROM propertyclaw_lease l WHERE {wc}", params).fetchone()[0]
...
def activate_lease(conn, args):
    ...
    conn.execute(
        "UPDATE propertyclaw_lease SET status = 'active', move_in_date = ?, updated_at = datetime('now') WHERE id = ?",
        (move_in, args.lease_id))

    # Auto-create base rent schedule if none exists
    if not conn.execute(
            "SELECT id FROM propertyclaw_rent_schedule WHERE lease_id = ? AND charge_type = 'base_rent'",
            (args.lease_id,)).fetchone():
        conn.execute(
            """INSERT INTO propertyclaw_rent_schedule
    ...
    audit(conn, SKILL, "prop-activate-lease", "propertyclaw_lease", args.lease_id,
...
def terminate_lease(conn, args):
    ...
    conn.execute(
        "UPDATE propertyclaw_lease SET status = 'terminated', move_out_date = ?, updated_at = datetime('now') WHERE id = ?",
        (args.move_out_date, args.lease_id))
    conn.execute("UPDATE propertyclaw_unit SET status = 'maintenance', updated_at = datetime('now') WHERE id = ?",
                 (lease["unit_id"],))

    audit(conn, SKILL, "prop-terminate-lease", "propertyclaw_lease", args.lease_id,
...
def list_rent_schedules(conn, args):
    if args.lease_id:
        ...
        rows = conn.execute(
            "SELECT * FROM propertyclaw_rent_schedule ORDER BY lease_id, charge_type").fetchall()
...
def generate_charges(conn, args):
    ...
    schedules = conn.execute(
        """SELECT * FROM propertyclaw_rent_schedule
    ...
    for sched in schedules:
        if conn.execute(
                "SELECT id FROM propertyclaw_lease_charge WHERE lease_id = ? AND charge_date = ? AND charge_type = ?",
                (args.lease_id, args.charge_date, sched["charge_type"])).fetchone():
            continue
        charge_id = str(uuid.uuid4())
        conn.execute(
            """INSERT INTO propertyclaw_lease_charge
...
def list_charges(conn, args):
    ...
    wc = " AND ".join(where)
    total = conn.execute(f"SELECT COUNT(*) FROM propertyclaw_lease_charge WHERE {wc}", params).fetchone()[0]
    limit = int(args.limit); offset = int(args.offset)
    rows = conn.execute(
        f"SELECT * FROM propertyclaw_lease_charge WHERE {wc} ORDER BY charge_date DESC, charge_type LIMIT ? OFFSET ?",
        params + [limit, offset]).fetchall()
...
def add_late_fee_rule(conn, args):
    ...
    try:
        conn.execute(
            """INSERT INTO propertyclaw_late_fee_rule
...
def list_late_fee_rules(conn, args):
    if args.company_id and args.state:
        ...
        rows = conn.execute(
            "SELECT * FROM propertyclaw_late_fee_rule WHERE company_id = ? ORDER BY state",
            (args.company_id,)).fetchall()
    else:
        rows = conn.execute(
            "SELECT * FROM propertyclaw_late_fee_rule ORDER BY state").fetchall()
...
def apply_late_fees(conn, args):
    ...
    leases = conn.execute(
        """SELECT l.*, p.state FROM propertyclaw_lease l
           JOIN propertyclaw_property p ON l.property_id = p.id
    ...
    for lease in leases:
        ...
        for charge in unpaid:
            if conn.execute(
                    "SELECT id FROM propertyclaw_lease_charge WHERE lease_id = ? AND charge_date = ? AND charge_type = 'late_fee' AND description LIKE ?",
...
def propose_renewal(conn, args):
    ...
    conn.execute(
        """INSERT INTO propertyclaw_lease_renewal
...
def accept_renewal(conn, args):
    ...
    new_name = get_next_name(conn, "propertyclaw_lease")
    ...
    conn.execute("UPDATE propertyclaw_lease SET status = 'renewed', updated_at = datetime('now') WHERE id = ?",
                 (old_lease["id"],))
    conn.execute("UPDATE propertyclaw_lease_renewal SET status = 'accepted', updated_at = datetime('now') WHERE id = ?",
                 (args.renewal_id,))
    ...
    audit(conn, SKILL, "prop-accept-renewal", "propertyclaw_lease", new_lease_id,
...
# ---------------------------------------------------------------------------
ACTIONS = {
    "prop-add-lease": add_lease,
    "prop-update-lease": update_lease,
    "prop-get-lease": get_lease,
    "prop-list-leases": list_leases,
    "prop-activate-lease": activate_lease,
    "prop-terminate-lease": terminate_lease,
    "prop-add-rent-schedule": add_rent_schedule,
    "prop-list-rent-schedules": list_rent_schedules,
    "prop-delete-rent-schedule": delete_rent_schedule,
    "prop-generate-charges": generate_charges,
    "prop-list-charges": list_charges,
    "prop-add-late-fee-rule": add_late_fee_rule,
    "prop-list-late-fee-rules": list_late_fee_rules,
    "prop-apply-late-fees": apply_late_fees,
    "prop-propose-renewal": propose_renewal,
    "prop-accept-renewal": accept_renewal,
}
