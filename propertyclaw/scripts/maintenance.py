#!/usr/bin/env python3
"""propertyclaw maintenance domain module.
...
REQUIRED_TABLES = ["company", "propertyclaw_property", "propertyclaw_work_order", "propertyclaw_inspection"]
SKILL = "prop-propertyclaw-maintenance"
...
def add_work_order(conn, args):
    ...
    wo_name = get_next_name(conn, "propertyclaw_work_order")
    ...
    audit(conn, SKILL, "prop-add-work-order", "propertyclaw_work_order", wo_id,
...
def get_work_order(conn, args):
    ...
    assignments = conn.execute(
        """SELECT va.*, s.name as vendor_name FROM propertyclaw_vendor_assignment va
...
def list_work_orders(conn, args):
    ...
    wc = " AND ".join(where)
    total = conn.execute(f"SELECT COUNT(*) FROM propertyclaw_work_order w WHERE {wc}", params).fetchone()[0]
...
def assign_vendor(conn, args):
    ...
    conn.execute(
        """INSERT INTO propertyclaw_vendor_assignment
    ...
    conn.execute(
        "UPDATE propertyclaw_work_order SET status = 'assigned', supplier_id = ?, updated_at = datetime('now') WHERE id = ?",
...
def complete_work_order(conn, args):
    ...
    conn.execute(
        """UPDATE propertyclaw_work_order
    ...
    conn.execute(
        """UPDATE propertyclaw_vendor_assignment
           SET status = 'completed', updated_at = datetime('now')
           WHERE work_order_id = ? AND status NOT IN ('completed', 'declined')""",
        (args.work_order_id,))

    audit(conn, SKILL, "prop-complete-work-order", "propertyclaw_work_order", args.work_order_id,
...
def add_work_order_item(conn, args):
    ...
    conn.execute(
        """INSERT INTO propertyclaw_work_order_item
...
def add_inspection(conn, args):
    ...
    conn.company_id = args.company_id
    insp_name = get_next_name(conn, "propertyclaw_inspection")

    conn.execute(
        """INSERT INTO propertyclaw_inspection
           (id, naming_series, company_id, property_id, unit_id, lease_id,
    ...
    audit(conn, SKILL, "prop-add-inspection", "propertyclaw_inspection", insp_id,
...
def list_inspections(conn, args):
    ...
    wc = " AND ".join(where)
    total = conn.execute(f"SELECT COUNT(*) FROM propertyclaw_inspection i WHERE {wc}", params).fetchone()[0]
...
def add_inspection_item(conn, args):
    ...
    item_id = str(uuid.uuid4())
    conn.execute(
        """INSERT INTO propertyclaw_inspection_item
...
# ---------------------------------------------------------------------------
ACTIONS = {
    "prop-add-work-order": add_work_order,
    "prop-update-work-order": update_work_order,
    "prop-get-work-order": get_work_order,
    "prop-list-work-orders": list_work_orders,
    "prop-assign-vendor": assign_vendor,
    "prop-update-vendor-assignment": update_vendor_assignment,
    "prop-complete-work-order": complete_work_order,
    "prop-add-work-order-item": add_work_order_item,
    "prop-list-work-order-items": list_work_order_items,
    "prop-add-inspection": add_inspection,
    "prop-get-inspection": get_inspection,
    "prop-list-inspections": list_inspections,
    "prop-add-inspection-item": add_inspection_item,
    "prop-list-inspection-items": list_inspection_items,
}
